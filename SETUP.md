...
## 3. WHOOP (legacy, currently orphaned)

> This integration predates the Garmin card on `index.html`, which now covers
> sleep/recovery/readiness instead. The `api/whoop-*.js` endpoints still exist,
> but the page they used to redirect back to (`health.html`) was retired, so
> this flow has no UI to land on today. Left here for reference only — wire the
> callback's redirect to a page of your own if you want to revive it.
...
3. Add these in Vercel → **Settings → Environment Variables**, then redeploy:
...
2. New Supabase → run the **SQL** above → paste your **URL + anon key** into `sync.js`,
   `topbar.js`, `gym.html`.
3. (Optional, legacy) WHOOP: the two env vars in Vercel — currently orphaned, see section 3.
